...
unsigned long charToInt(unsigned char* arr, int size) {
    ...
    kq |= (unsigned long)temp[0] << shifter; // Copy byte dau vao ben phai cung
    shifter += 8;
    for (int i = 1; i < size; i++) {
        kq |= (unsigned long)temp[i] << shifter; // copy cac byte con lai lui dan ve ben trai
        shifter += 8;
    }
    if (flag == 1) {
        kq += (unsigned long)pow(2, 31);
    }
    delete[] temp;
    return kq;
}
